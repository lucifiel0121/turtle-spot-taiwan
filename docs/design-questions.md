...
- **實作採用的詮釋**：忠實渲染外觀（含喇叭 icon 為比 ink 底更深的純黑），不接任何行為。
...
### 19. Sounds icon 純黑疊 ink 深底，對比 1.09:1（WCAG 1.4.11）

- **疑義描述**：navbar 喇叭 icon 設計稿為純黑，疊在 ink 深色底上量測對比僅約 1.09:1；WCAG 1.4.11（Non-text Contrast）要求 UI 元件的視覺資訊至少 3:1，該鈕幾乎不可見。
- **實作採用的詮釋**：忠實重現設計稿的純黑 icon（`text-black`，元件註解「比 ink 底更深」），不擅自改色。
- **依據／量測**：`components/navbar.tsx`（Volume2 icon）；Desktop 1440 截圖取樣 icon 與底色計算對比。
- **建議提案（給設計方）**：icon 改用 foam／白色（與同列 EN、MENU 一致）或任何與 ink 底對比 ≥ 3:1 的色值。
- **澄清後調整成本**：極低 — 改一個 className 色值。
